...
class binaryHeap {
	...
	public:
		...
		void minHeapify(node* a, int pos, int len) {
			int i = 0;
			if (pos*2 + 1 < len && a[pos].weight > a[pos*2+1].weight) i = pos*2 + 1;
			else i = pos;
			if (pos*2 + 2 < len && a[i].weight > a[pos*2+2].weight) i = pos*2 + 2;
			if (i == pos) return;
			node k = a[i];
			a[i] = a[pos];
			a[pos] = k;
